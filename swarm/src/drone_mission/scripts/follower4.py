...
# =====================================================
# MAVLINK CONNECTION (FOLLOWER 4)
# =====================================================
# Connects to ArduPilot SITL instance 4 via UDP.
...
connection = mavutil.mavlink_connection('udp:127.0.0.1:14590')
print("Connecting Follower4...")
connection.wait_heartbeat()
print("Follower4 connected")


# =====================================================
# FORMATION / CONTROL PARAMETERS
# =====================================================
# Follower4 uses a larger offset (same magnitude as follower3),
# but it sits on the RIGHT side of the leader (mirror of follower3).
#
# NED frame reminder:
...
DIST_SIDE = 10.0         # Desired lateral offset to the RIGHT (meters)
...
def start_callback(msg: String):
    ...
    if msg.data.strip().lower() == "start":
        ...
        print("[SYNC] Follower4 START received → formation active")


def leader_velocity_callback(msg: Point):
    """
    Main formation controller (Follower4):

    Mirror of follower3 (right side with large offsets).
...
def main():
    """
    Follower4 node lifecycle:
      1) MAVLink: set GUIDED, arm, takeoff
      2) ROS2: publish TF + Path for RViz
      3) ROS2: synchronize via READY/START
      4) ROS2: follow leader velocity + formation geometry (RIGHT + larger offset)
      5) ROS2: respond to swarm LAND command

    Note:
      Before START, this script optionally sends zero velocity commands
      to keep the drone steady (helps avoid any stale velocity command).
    """
    global formation_active
    ...
    # ---------- ROS2 INITIALIZATION ----------
    rclpy.init()
    node = rclpy.create_node('follower4_control')

    # RViz visualization publishers
    path_pub = node.create_publisher(Path, '/follower4_path', 10)
    ...
    # Send READY burst (important during startup)
    ready_msg = String()
    ready_msg.data = "follower4"
    for _ in range(10):
        ready_pub.publish(ready_msg)
        time.sleep(0.05)
    print("[SYNC] READY sent: follower4")
    ...
    def publish_tf_and_path():
        """
        Publishes:
          - TF transform for follower4/base_link
        ...
        t.child_frame_id = "follower4/base_link"
    ...
    print("Follower4 ready (waiting for START...)")
    rclpy.spin(node)
