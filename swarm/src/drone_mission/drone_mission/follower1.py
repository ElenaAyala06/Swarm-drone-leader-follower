...
# =====================================================
# MAVLINK CONNECTION (FOLLOWER 1)
# =====================================================
# Connects to ArduPilot SITL instance 1 via UDP.
# By convention in your launch:
#   leader   -> 14550
#   follower1-> 14560
# =====================================================

connection = mavutil.mavlink_connection('udp:127.0.0.1:14560')
print("Connecting Follower1...")
...
DIST_SIDE = 5.0          # Desired lateral offset to the left (meters)
...
def leader_velocity_callback(msg: Point):
    """
    Main formation controller (Follower1):
...
def main():
    ...
    # ---------- ROS2 INITIALIZATION ----------
    rclpy.init()
    node = rclpy.create_node('follower1_control')

    # RViz visualization publishers
    path_pub = node.create_publisher(Path, '/follower1_path', 10)
    ...
    ready_msg = String()
    ready_msg.data = "follower1"
    for _ in range(10):
        ready_pub.publish(ready_msg)
        time.sleep(0.05)
    print("[SYNC] READY sent: follower1")
    ...
    def publish_tf_and_path():
        """
        Publishes:
          - TF transform for follower1/base_link
        ...
        t.child_frame_id = "follower1/base_link"
    ...
    print("Follower1 ready (TF + Path + Sync)")
    rclpy.spin(node)
